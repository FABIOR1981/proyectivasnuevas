...
Si `index.html` se abre sin una configuración activa, avisa y ofrece ir a `configuracion.html`. Durante la evaluación no hay forma de cambiar los parámetros desde la propia página: el evaluador debe volver a `configuracion.html`. Esto es una **separación de uso, no una medida de seguridad**: cualquiera que conozca la dirección puede abrir la página de configuración. Restringirla de verdad requeriría autenticación en un servidor.
...
### Capa de almacenamiento (`js/storage.js`)
...
- **`JsonProfilesStore`**: agrega perfiles compartidos desde un archivo JSON del sitio (solo lectura) y delega los perfiles nuevos y la configuración activa en otra implementación. No está activada; para usarla se cambia la última línea de `storage.js`:
...
Para una **base de datos**, se escribe otra clase con el mismo contrato (en `storage.js` hay un ejemplo comentado de `ApiStore` sobre una API REST) y se elige en esa misma línea. Ninguna otra parte del código cambia.
...
├── css/
│   └── styles.css      Estilos compartidos (incluye vista de impresión)
├── js/
│   ├── common.js       Utilidades, modelo de configuración y generación de laberintos
│   ├── storage.js      Capa de almacenamiento (contrato + implementaciones)
│   ├── config.js       Lógica de configuracion.html
│   └── app.js          Lógica de index.html
└── README.md
```

`app.js` se organiza en secciones: utilidades y flujo principal, Casa, Constelación, Dinámica Cromática, Laberinto (dibujo, análisis, pantalla completa y registro del recorrido), aplicación de la configuración (cronómetro y límites), guardado e informe.
